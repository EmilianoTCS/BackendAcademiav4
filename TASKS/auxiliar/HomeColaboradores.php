...
// TOTAL PORCENTAJE
$queryPorcentaje = mysqli_query($conection, "SELECT ROUND((COUNT(ap.porcentaje_aprobacion)* 100) / (SELECT COUNT(ID) from ramos),1) as porcentaje from aprobacion ap WHERE ap.porcentaje_aprobacion > 90 AND ap.usuario = 'asagredo'");
...
$queryPendientes = mysqli_query($conection, "SELECT round(count(ID) - (SELECT COUNT(ram.ID) as totalFinalizados from ramos ram 
											 INNER JOIN cursos cur, evaluaciones eva 
											 WHERE ram.ID = cur.idRamo AND cur.codigoCurso = eva.codigoCurso AND eva.estado = 'Enviado' AND eva.usuario = 'asagredo')) as totalPendientes from ramos"); 
...
// PROMEDIO
$queryPromedio = mysqli_query($conection, "SELECT ROUND(AVG(nota),1) as promedio from evaluaciones WHERE usuario = 'asagredo'");
