...
if (isset($_GET['updateStateRelator'])) {
  ...
  $query = "UPDATE relator SET isActive = !isActive, fechaActualizacion = '$date' WHERE ID = '$ID'";
  $result = mysqli_query($conection, $query);

  if (!$result) {
    die(json_encode('Query Failed.'));
  }
  $query2 = "SELECT ID, nombre, isActive, fechaActualizacion from relator WHERE ID = '$ID'";
