...
if (isset($_GET['pagina'])) {
    ...
    $query = "SELECT ram.*, cur.*, cuen.codigoCuenta, IF(cur.fin < date(CURRENT_DATE), 'Finalizado', IF(cur.inicio < date(CURRENT_DATE) and CURRENT_DATE < cur.fin, 'En curso', IF(CURRENT_DATE < cur.inicio, 'Pendiente', ''))) as estado FROM cursos cur INNER JOIN ramos ram, cuentas cuen WHERE cur.idCuenta = cuen.ID AND cur.codigoRamo = ram.codigoRamo AND cur.isActive = true order by cur.ID ASC LIMIT $inicio,$cantidad_por_pagina ";
