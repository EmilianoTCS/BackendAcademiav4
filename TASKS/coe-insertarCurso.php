    ...
    if (!empty($codigoRamo) && !empty($fechas)) {
        ...
        for ($i = 0; $i < $longitud; ++$i) {
            ...
            $horaFin = date('H:i:s', ($hora1 + $duracion));
                ...
                $queryVerify = "SELECT * FROM cursos WHERE codigoRamo = '$codigoRamo' AND fecha_hora = '$fechas[$i]' AND sesion = '$sesion' AND hora_inicio <= time('$formatTimeTemporal') AND hora_fin >= time('$horaFin') ";
                ...
                if (mysqli_num_rows($resultVerify) >= 1 ) {
                    ...
                    if (strtotime($primerElemento) > strtotime(date('Y-m-d H:i:s', time())) && strtotime(date('Y-m-d H:i:s', time())) < strtotime($ultimoElemento)) {
                        ...
                        $query = "INSERT INTO cursos (idCuenta, idRamo, codigoCurso, codigoRamo, fecha_hora, sesion, inicio, fin, hora_inicio, hora_fin, isActive, fechaActualizacion) VALUES ('$codigoCuenta',(SELECT ID from ramos WHERE codigoRamo = '$codigoRamo'), '$codigoCurso','$codigoRamo','$fechas[$i]','$sesion','$fechaInicio', '$fechaFin', '$formatTimeTemporal', '$horaFin', true, current_timestamp());";
